...
struct SimParams
{
    ...
    // Each device's OWN currently-selected frequency, always a value native
    // to (or interpolated only within) THAT model's own prescribedFrequencies
    // — never borrowed from the other model. Every speaker picks its
    // directivity pattern using its own model's entry here, never the
    // other model's, and never the raw `frequency` above when that belongs
    // to a different model. Q21S and 15W750 are isolated by construction:
    // changing one can never change the other's value or its speakers'
    // rendered pattern. Populated by ControlPanel::getParams() every call.
